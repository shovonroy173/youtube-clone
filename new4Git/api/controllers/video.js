...
const addVideo = async (req, res, next) => {
    ...
    const videoBody = {title , desc , imgUrl , videoUrl};
    console.log(title);
  const newVideo = new Video({ userId: req.user.id , ...videoBody} );
...
const search = async (req, res, next) => {
    ...
    try {
        const videos = await Video.find({title:  { $regex: query , $options:"i" }}).sort({views:1});
        res.status(200).json(videos)
    } catch (error) {
        console.log(error);
        // next(error);
...
module.exports = {dislikes , likes , latest , addVideo , updateVideo , deleteVideo , getVideo , addView , random , trend , subscribed , getByTag , search };
